...
handle_ring(Apid, _RingData, Callrec, State) ->
	...
	case freeswitch_ring:start(State#state.cnode, AgentRec, Apid, Callrec, ?getRingout, F) of
		{ok, Pid} ->
			link(Pid),
...
handle_warm_transfer_begin(Number, Call, #state{agent_pid = AgentPid, cnode = Node, ringchannel = undefined} = State) when is_pid(AgentPid) ->
	case freeswitch:api(Node, create_uuid) of
		{ok, NewUUID} ->
			...
			case freeswitch_ring:start(Node, AgentState, AgentPid, Call, ?getRingout, F, [no_oncall_on_bridge, {eventfun, F2}, {needed_events, ['CHANNEL_BRIDGE']}]) of
				{ok, Pid} ->
					link(Pid),
...
handle_warm_transfer_cancel(Call, #state{warm_transfer_uuid = WUUID, cnode = Node, agent_pid = AgentPid} = State) when is_list(WUUID) ->
	case freeswitch:api(Node, create_uuid) of
		{ok, NewUUID} ->
			...
			case freeswitch_ring:start(Node, AgentState, AgentPid, Call, ?getRingout, F, []) of
				{ok, Pid} ->
					link(Pid),
...
handle_call({dial, Number}, _From, Call, #state{cnode = Fnode, dialstring = DialString, agent_pid = Apid} = State) ->
	...
	case freeswitch_ring:start(Fnode, AgentRec, Apid, Call, ?getRingout, F, [no_oncall_on_bridge, {eventfun, F2}, {needed_events, ['CHANNEL_BRIDGE']}]) of
		{ok, Pid} ->
			link(Pid),
